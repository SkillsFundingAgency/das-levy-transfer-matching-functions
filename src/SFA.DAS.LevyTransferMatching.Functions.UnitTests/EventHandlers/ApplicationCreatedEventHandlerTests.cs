...
namespace SFA.DAS.LevyTransferMatching.Functions.UnitTests.EventHandlers
{
    ...
    {
        private ApplicationCreatedEventHandler _handler;
        ...
        public void Setup()
        {
            ...
            _handler = new ApplicationCreatedEventHandler(_legacyTopicMessagePublisher.Object, Mock.Of<ILogger>());
        }

        [Test]
        public async Task Run_Invokes_Legacy_Topic_Message_Publisher()
        {
            var sourceEvent = _fixture.Create<ApplicationCreatedEvent>();

            await _handler.Handle(sourceEvent, Mock.Of<IMessageHandlerContext>());

            _legacyTopicMessagePublisher.Verify(x => x.PublishAsync(It.Is<PledgeApplicationCreated>(r =>
                r.PledgeId == sourceEvent.PledgeId &&
                r.ApplicationId == sourceEvent.ApplicationId &&
                r.CreatedOn == sourceEvent.CreatedOn &&
